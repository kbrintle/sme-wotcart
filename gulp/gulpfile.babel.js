...
import { pkg, paths, serverrc } from '../gulp/gulp.config';
...
export const css = () => src( cssPaths.concat.src )
	.pipe( pkg.concat( cssPaths.concat.name ) )
...
export const transpile = ( source, name, destination ) => src( source )
	...
	.pipe( pkg.if( PRODUCTION, pkg.uglify() ) )
	.pipe( dest( destination ) );

export const js = () => {
	const streams = pkg.mergeStream();
	streams.add( transpile( jsPaths.admin.src, jsPaths.admin.name, jsPaths.admin.dest ) );
	streams.add( transpile( jsPaths.frontend.src, jsPaths.frontend.name, jsPaths.frontend.dest ) );
...
export const watchForChanges = () => {
	...
	watch( jsPaths.frontend.src, series( js, reload ) );
